...
const MyWalletScreen = ({ navigation }) => {
  ...
  if (!walletSummary) {
    return (
      <View style={styles.centered}>
        <Text style={styles.errorText}>{errorMsg || 'Failed to load wallet'}</Text>
        <Text style={styles.emptySubtext}>{(errorMsg && errorMsg.includes('verification')) ? 'Please verify your student profile first.' : ''}</Text>
      </View>
    );
  }
