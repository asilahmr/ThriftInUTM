  ...
  const pickImage = async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Denied",
          "We need camera roll permissions to attach evidence.",
        );
        return;
      }
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      const fetchPromise = fetch(
        "https://thriftinutm-production.up.railway.app/api/reports/submit",
        {
          method: "POST",
          headers: {
      ...
      <ScrollView style={styles.content}>
        ...
        {["Spam", "Harassment", "Scam", "Other"].map((r) => (
          ...
          >
            ...
            <Text
              style={[styles.optText, selectedReason === r && styles.selText]}
            >
              {r}
            </Text>
          </TouchableOpacity>
